//! Every balance row has exactly one writer. Leaf accounts and synchronous
//! account sets are written by the poster, inline in the posting
//! transaction. Eventually-consistent sets are excluded from that fold and
//! are written only by the balance projector (see [`crate::projector`]),
//! whose outbox cursor is the sole progress counter for them.
...
impl Balances {
    ...
    #[instrument(
        name = "cala_ledger.balance.load_account_set_balances_batch",
        skip(self, op),
        err(level = "warn")
    )]
    ...
    #[instrument(
        name = "cala_ledger.balance.insert_new_snapshots",
    ...
    /// Rebuild the cumulative-by-date dimension of the given EC sets from
    /// `min_effective_date` forward.
    #[instrument(
        name = "cala_ledger.balance.rebuild_effective_balances_from_date_in_op",
        skip(self, op),
        err(level = "warn")
    )]
    pub(crate) async fn rebuild_effective_balances_from_date_in_op(
        &self,
        op: &mut impl es_entity::AtomicOperation,
        journal_id: JournalId,
        ...
        let memberships = self
            ...
            .await?;
        self.effective
            .rebuild_for_account_sets_from_date_in_op(
                op,
                journal_id,
                account_set_ids,
                &memberships,
    ...
    /// `cala_balance_history` for `journal_id`.
    #[instrument(
        level = "debug",
